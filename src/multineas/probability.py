...
from .orbit import transformation_x_to_e, compute_jacobian_x_to_e
...
class OrbitElementsPDF:
    ...
    def __init__(self, fit_mog: mn.FitMoG, scales: Union[List[float], np.ndarray] = None):
        """
    ...
    def compute_p_e_cmnd(self, q: float, e: float, i: float) -> float:
        ...
        u_element = mn.Util.t_if(element, self.scales, mn.Util.f2u)
        
        # Compute CMND PDF for (q, e, i)
        P_qei = self.fit_mog.mog.pdf(u_element)
...
class PhaseSpacePDF:
    ...
    def __init__(self, orbit_coords: np.ndarray, orbit_elements_pdf: OrbitElementsPDF,
    ...
    def compute_p_x_cmnd(self, x: Union[float, np.ndarray], y: Union[float, np.ndarray],
        ...
        for idx in range(x_flat.size):
            ...
            q, e, i, Omega, w, M, a = transformation_x_to_e(
            ...
            jacobian_x_to_e = compute_jacobian_x_to_e(
...
def compute_jacobian_qei_to_QEI(q: float, e: float, i: float,
    ...
    >>> 
    >>> q, e, i = 0.5, 0.3, np.pi/4
    >>> q_max, e_max, i_max = 1.30, 1.0, np.pi
